...
- Tests that need to scroll past the hero must scroll to at least `window.innerHeight * PIN_MULTIPLIER` (un-pin point) — use `* (PIN_MULTIPLIER + 1.5)` for safety margin past the nav (currently `* 9.5`).
